...
import (
	...
	"github.com/aceransh/aegis/internal/db"
...
func main() {
	...
	http.HandleFunc("/enqueue", func(w http.ResponseWriter, r *http.Request) {
		...
		if idemKey != "" {
			...
			if rows == 0 {
				...
				err := db.DB.QueryRow("SELECT status, job_id FROM Idems WHERE idem_key = $1", idemKey).Scan(&status, &existingJobID)
				if err != nil {
	...
	http.HandleFunc("/ack", func(w http.ResponseWriter, r *http.Request) {
		...
		tx, err := db.DB.Begin() //setup a transaction
		if err != nil {
